...
- **`vpk.exe x` 遇到列表里第一个不存在的文件就会停止处理后续文件**（只打印一行 `couldn't find file ...`）。所以一个批次里只要混进一个该包里没有的文件，排在它后面的文件全都不会被解出来。给一批文件跨多个包解包时，务必**按包分组**，每组只喂这个包里确实存在的文件；否则表现是一批模型"没解出来"，随后被误判成转换失败。
